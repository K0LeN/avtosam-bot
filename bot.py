...
async def got_block(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔧 {block} — რა სერვისი?",
        reply_markup=make_keyboard(service_list, cols=1)
    )
    return WAIT_SERVICE
...
async def got_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price_type == "select":
        ...
        await update.message.reply_text(
            "💰 ფასი:",
            reply_markup=make_keyboard(prices, cols=4)
        )
        return WAIT_PRICE_SELECT
    elif price_type == "oil":
        await update.message.reply_text(
            "🛢 რამდენი ლიტრი?",
            reply_markup=make_keyboard(["1", "2", "3", "4", "5", "6", "7", "8"], cols=4)
        )
...
async def got_price_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if mode == "debt":
            await update.message.reply_text(
                "💳 რამდენი გადაიხადა ახლა? (0 თუ არ გადაუხდია)",
                reply_markup=make_keyboard(["0"], cancel=True)
            )
...
async def got_oil_liters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "🔢 სიბლანტე:",
            reply_markup=make_keyboard(OIL_VISCOSITIES, cols=2)
        )
...
async def ask_employee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👤 ვინ შეასრულა?",
        reply_markup=make_keyboard(employees, cols=1)
    )
...
async def got_employee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "💯 რამდენი პროცენტი?",
        reply_markup=make_keyboard(["10", "15", "20", "25", "30", "35", "40", "50"], cols=4)
    )
    return WAIT_PERCENT
...
async def got_percent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            summary,
            reply_markup=make_keyboard(["✅ დადასტურება", "❌ გაუქმება"], cols=2)
        )
        return WAIT_CONFIRM
    except ValueError:
        await update.message.reply_text("გთხოვ ციფრი შეიყვანე!")
...
async def got_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text == "✅ დადასტურება":
        d = context.user_data
        try:
            row = add_service_record(
                d["block"], d["car_number"], d["service"],
                d.get("details", ""), d["price"], d["employee"], d["percent"]
            )
            await update.message.reply_text(
                f"✅ ჩაიწერა! #{row}\n🚗 {d['car_number']} | {d['service']}\n"
                f"💰 {d['price']} ₾ | 👤 {d['employee']}"
            )
...
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📸 სერვისის ჩაწერა":
        ...
        actions = ["➕ სერვისის დამატება", "👤 თანამშრომლის დამატება",
                   "📋 სერვისების სია", "👥 თანამშრომლების სია"]
...
async def send_report(update: Update, context: ContextTypes.DEFAULT_TYPE, date_str=None):
    try:
        ...
        text = (
            f"📊 რეპორტი — {date}\n\n"
            f"🚿 სამრეცხაო: {report['სამრეცხაო']:.0f} ₾\n"
            f"⭐ სერვისი: {report['სერვისი']:.0f} ₾\n"
            f"💰 სულ: {total_income:.0f} ₾\n\n"
            f"💸 ხარჯები: {report['ხარჯები']:.0f} ₾\n"
            f"📈 სუფთა მოგება: {net_profit:.0f} ₾\n\n"
...
async def daily_report_job(context):
    try:
        ...
        emp_text = "".join(f"  👤 {e}: {a:.0f} ₾\n" for e, a in report.get("employees", {}).items())
        text = (
            f"🌙 დღიური რეპორტი — {date}\n\n"
            f"🚿 სამრეცხაო: {report['სამრეცხაო']:.0f} ₾\n"
            f"⭐ სერვისი: {report['სერვისი']:.0f} ₾\n"
            f"💰 სულ: {total_income:.0f} ₾\n"
            f"💸 ხარჯები: {report['ხარჯები']:.0f} ₾\n"
            f"📈 მოგება: {net_profit:.0f} ₾\n\n"
